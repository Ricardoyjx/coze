...
from config import (
    app, COZE_SPACES, COZE_API_TOKEN,
    JD_WORKFLOW_ID, SCREEN_WORKFLOW_ID, OFFER_WORKFLOW_ID,
    uploaded_resumes, screening_tasks, jd_history, ResumeData,
)
from models import (
    JDGenerateRequest, ScreeningRequest, OfferEmailRequest,
    SalaryAnalysisRequest, BatchScreenRequest,
)
...
async def generate_offer_email(req: OfferEmailRequest):
    return StreamingResponse(
        offer_stream_generator(
            req.candidate_name, req.position, req.salary,
            req.start_date, req.location, req.notes or "", req.company_name or "",
        ),
        media_type="text/plain",
        headers={"Cache-Control": "no-cache", "X-Content-Type-Options": "nosniff"},
    )
...
async def analyze_salary(req: SalaryAnalysisRequest):
    ...
    mock_data = {
        ...
        "experience": req.experience or "3-5年", "education": req.education or "本科",
        "salaryRange": {"min": 15, "max": 45, "median": 25},
        "percentiles": {"p10": 12, "p25": 18, "p50": 25, "p75": 32, "p90": 40},
        "industryAvg": 26, "cityAvg": 24,
        "experienceLevels": [
            {"level": "1年以下", "salary": 10}, {"level": "1-3年", "salary": 16},
            {"level": "3-5年", "salary": 22}, {"level": "5-10年", "salary": 30},
            {"level": "10年以上", "salary": 40},
        ],
        "educationImpact": [
            {"level": "大专", "salary": 18}, {"level": "本科", "salary": 25},
            {"level": "硕士", "salary": 32}, {"level": "博士", "salary": 40},
        ],
        "recommendedRange": "22K - 32K", "confidence": "高",
    }
    ...
    if ratio != 1.0:
        ...
        mock_data["recommendedRange"] = f"{round(int(lo) * ratio)}K - {round(int(hi) * ratio)}K"
...
async def batch_screen(req: BatchScreenRequest):
    ...
    for i, (name, tech, skills_str, score, rec) in enumerate(selected, 1):
        ...
        results.append({
            "id": f"batch_{i}", "name": name, "filename": f"{name}_简历.pdf",
            "mainTech": tech, "skills": skills_str, "score": score,
            "recommendation": rec, "passed": passed, "education": "本科", "workYears": 3 + (i % 5),
        })
    passed_count = sum(1 for r in results if r["passed"])
    return {
        "total": len(results), "threshold": req.threshold,
        "passedCount": passed_count, "failedCount": len(results) - passed_count,
    ...
    async def event_generator():
        ...
        for i in range(total):
            ...
            result_item = {
                "id": resume_id,
                "resume": {
                    "id": resume_id, "filename": f"{name}_简历.pdf", "name": name,
                    "phone": f"138{str(i+1).zfill(8)}", "email": f"{name}@email.com",
                    "education": edu, "workYears": work_years, "skills": skills,
                    "experience": f"{work_years}年{tech}开发经验",
                    "uploadedAt": datetime.now().isoformat(),
                },
                "score": score, "matchPoints": matches, "missingPoints": misses,
            ...
            progress_data = {
                ...
                "totalResumes": total, "processedResumes": i + 1,
                "results": results, "createdAt": datetime.now().isoformat(),
            }
...
async def coze_status():
    ...
    for space_id, cfg in COZE_SPACES.items():
        space = {
            "space_id": space_id, "name": cfg.get("name", ""),
            "api_key_configured": bool(cfg.get("api_key")),
            "bot_id": cfg.get("bot_id", ""), "bots": [],
        }
        client = get_coze_client(space_id)
        if not client:
            space["error"] = "API Key 未配置" if not cfg.get("api_key") else "客户端初始化失败"
            spaces.append(space)
            continue
        try:
    ...
    return {
        "spaces_configured": len(COZE_SPACES), "spaces": spaces,
        "workflows": {
            ...
            "offer_email": {"id": OFFER_WORKFLOW_ID, "configured": bool(OFFER_WORKFLOW_ID)},
        },
